...
export function Bufferfrom(obj: Uint8Array) {
...
export class FS implements IFileSystem {
  ...
  public readFile = async (
    ...
    if (existingRecord) {
      ...
      const data = Bufferfrom(existingRecord.data)
      const result = data && options.encoding ? data.toString(options.encoding as any) : data
  ...
  public readFileSync(filename: string, arg2: string | { encoding: string; flag?: string }): any {
    const options = normalizeOptions(arg2, null, 'r', null)
    const existingRecord = (
      this.adapter.query({
        id: normalizePath(filename),
  ...
  public writeFile = async (
    ...
    try {
      const payload = this._inputToNodeRecord(filename, data, options, existingRecord)
  ...
  mkdir = async (p: string, mode?: any, cb: BFSOneArgCallback = nopCb): Promise<void> => {
    const path = p
   ...
   * @param callback
   */
  link = async (srcpath: string, dstpath: string, cb: BFSOneArgCallback): Promise<void> => {
   ...
   * @param callback
   */
  symlink = async (srcpath: string, dstpath: string, type?: string, cb?: BFSOneArgCallback): Promise<void> => {
