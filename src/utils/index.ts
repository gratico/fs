...
export async function mkdirP(fs: IFileSystem, p: string, mode?: any): Promise<void> {
...
export function mkdirPFunction(p: string, opts: { fs?: any; mode?: any }, f: any, made?: any) {
...
export function digestBuffer(buffer: Buffer) {
  return checksum(buffer.toString())
}

export function normalizeMode(mode: number | string | null | undefined, def: number): number {
  switch (typeof mode) {
    case 'number':
      // (path, flag, mode, cb?)
      return <number>mode
